    ...
    Future<void> addNode() async {
      ...
      try {
        ...
        try {
          ...
          client = KobraClient.url(url, isSecure: true);
          nodeInfo = await client.getInfo();
          networkName = (await client.getBlockDagInfo()).networkName;
          isSecure = true;
        } catch (_) {
          // Fallback to insecure connection
          client = KobraClient.url(url, isSecure: false);
          nodeInfo = await client.getInfo();
          networkName = (await client.getBlockDagInfo()).networkName;
          isSecure = false;
        }

        KobraNetwork network;
        String suffix;
        final parts = networkName.split('-');
        if (parts.length > 1) {
          network = KobraNetwork.tryParse(parts[1]) ?? networkForPort(port);
        ...
        final notifier = ref.read(kobraNodeSettingsProvider.notifier);
