...
final _kobraPriceRemoteProvider =
  ...
  final cached = ref.read(_kobraPriceCacheProvider);
  ...
  try {
    ...
    log.e('Failed to fetch KODA exchange rate', error: e, stackTrace: st);
    if (cached.currency == currency.currency) {
      return cached;
    }
    return CoinGeckoPrice(
...
final coingeckoKobraPriceProvider = Provider.autoDispose((ref) {
  final cache = ref.watch(_kobraPriceCacheProvider.notifier);
  final remote = ref.watch(_kobraPriceRemoteProvider);
