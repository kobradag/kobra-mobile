...
final kobraPriceProvider = Provider.autoDispose((ref) {
  return ref.watch(coingeckoKobraPriceProvider);
});
...
final formatedTotalFiatProvider = Provider.autoDispose((ref) {
  final balance = ref.watch(totalBalanceProvider);
  final price = ref.watch(kobraPriceProvider);
...
final formatedkobraPriceProvider = Provider.autoDispose((ref) {
  final price = ref.watch(kobraPriceProvider).price;
  final currency = ref.watch(currencyProvider);
...
final kobraFormatterProvider = Provider((ref) {
  final symbol = ref.watch(kasSymbolProvider);
  ...
  final formatter = CurrencyFormatter(
    ...
    maxDecimalDigits: TokenInfo.kobra.decimals,
    maxAmount: kMaxKobra,
  );
...
final fiatFormatterProvider = Provider.autoDispose((ref) {
  final price = ref.watch(kobraPriceProvider);
  final currency = ref.watch(currencyProvider);
  final maxAmount = price.price * kMaxKobra;
