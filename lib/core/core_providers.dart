...
final networkProvider = Provider((ref) {
  final config = ref.watch(kobraNodeConfigProvider);
...
final _KobraApiProvider = Provider<KobraApi>((ref) {
  final networkId = ref.watch(networkIdProvider);

  return switch (networkId) {
    kKobraNetworkIdMainnet =>
      KobraApiMainnet('https://api.k0bradag.com'),
...
final KobraApiServiceProvider = Provider<KobraApiService>((ref) {
  final api = ref.watch(_KobraApiProvider);
  return KobraApiService(api);
});

final kobraClientProvider = Provider((ref) {
  final config = ref.watch(kobraNodeConfigProvider);
  final inBackground = ref.watch(inBackgroundProvider);

  final client = inBackground
      ? VoidKobraClient()
      : KobraClient.url(config.url, isSecure: config.isSecure);
...
final balancesForAddressesProvider = FutureProvider.family
    .autoDispose<Iterable<RpcBalancesByAddressesEntry>, List<String>>(
        (ref, addresses) async {
  final client = ref.watch(kobraClientProvider);
...
final symbolProvider = Provider.family<String, Amount>((ref, amount) {
  ...
  if (amount.tokenInfo.tokenId != TokenInfo.kobra.tokenId) {
